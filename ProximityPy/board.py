...
class Board(object):
    ...
    def __init__(self, height=HEIGHT, width=WIDTH):
        ...
        self.height = height
        self.width = width
        # tuples of x, y coords for free tiles
        self.available = [x for x in cartesianProduct(range(height), range(width))]
    ...
    def isAvailable(self, x, y):
        return (x, y) in self.available

    def move(self, player, x, y, value):
        if not self.isAvailable(x, y):
            return False
        else:
            self.available.remove((x, y))
            self.grid[x, y, UNCLAIMED] = 0
            self.grid[x, y, player] = value
            self.captureNeighbors(player, x, y, value)
            return True
    ...
    def evalMove(self, player, row, col, value):
    ...
    def getScore(self, player):
        ...
        return sum(self.grid[:, :, player][self.grid[:, :, player] > 0])
