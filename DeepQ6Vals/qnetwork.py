...
# INPUT KEYS
INPUT_KEYS = ['boardInput', #'remainingMovesInput',
...
def model():
    ...
    ################################################################################################
    model = Model(inputs=[boardInput, #remainingMovesInput,
                          scoreInput, moveValueInput], outputs=ouputLayer)
...
# #
# network.fit(x=dict(zip(INPUT_KEYS,
#     [fakeBoard, # fakeMoves,
#     fakeScores, fakeMoveValue])), y=fakeTarget)
# #
# y = network.predict(x=dict(zip(INPUT_KEYS,
#     [fakeBoard, #fakeMoves,
#      fakeScores, fakeMoveValue])), batch_size=1)
# print(y[0])
